...
void randomPowerups(game_data &game)
{
    ...
    powerup_type powerupTypes[] = {SHIELD, SPEED_BOOST, SLOW_DOWN, COIN}; // Array of powerup types
...
{
    // random barricades at random y locations

    int yLocations[] = {-500, -1000, -1500, -2000,}; // Array to store the y locations

    // Randomly select y locations for the barricades
    for (int i = 0; i < BARRICADE_COUNT; i++)
    {
        int randomIndex = rand() % 4; // Generate a random index between 0 and 6
        int y = yLocations[randomIndex];
        barricade_data barricade = new_barricade(1, y); // Create a barricade sprite at the selected y location
        game.barricades.push_back(barricade);           // Add the barricade to the game's barricade vector
    }
    ...
    if (game.twoPlayer)
    {
        for (int i = 0; i < game.powerups.size(); i++)
        {
            if (sprite_collision(game.car.car_sprite, game.powerups[i].powerup_sprite) || sprite_collision(game.secondPlayer.car_sprite, game.powerups[i].powerup_sprite))
            {
                ...
                std::string powersounds[] = {"powerup1", "powerup2"};
...
void level_up(game_data &game)
{
    ...
    if (game.score >= 2000 && game.score <= 2100)
    {
        game.finish_line = new_finish_line();
        game.finished = true;
    }
    if (game.score >= 2500 && game.score <= 2600)
    {
        game.finish_line = new_finish_line();
        game.finished = true;
    }
